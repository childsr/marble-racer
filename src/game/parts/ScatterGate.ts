...
export function createScatterGate(
  ...
  const leftBumper = scene.matter.add.trapezoid(x - w / 2 + bumperW/2, y, bumperW, fixedH, 0.5, {
    isStatic: true,
    friction: 0,
    restitution: 0.5,
  ...
  const rightBumper = scene.matter.add.trapezoid(x + w / 2 - bumperW/2, y, bumperW, fixedH, 0.5, {
    isStatic: true,
    friction: 0,
    restitution: 0.5,
    label: "scatter_gate_bumper",
  ...
  const mainSensor = scene.matter.add.rectangle(x, y, w - 20, 6, {
    isSensor: true,
    isStatic: true,
